...
while robot.step(SIM_TIMESTEP) != -1:
    ...
    if state == 'speed_measurement':
        ...
        # if center_sensor and right_sensor and left_sensor:
        #     vL = -leftMax
        #     vR = rightMax
        # if center_sensor and left_sensor:
        #     vL = -leftMax * .05
        #     vR = rightMax
        # elif center_sensor and right_sensor:
        #     vL = leftMax 
        #     vR = -rightMax * .05
        # elif center_sensor: #go straight
        #     vL = leftMax
        #     vR = rightMax
        # elif left_sensor:#move counterclockwise in place
        #     vL = -leftMax
    ...
    pose_theta += (((vR/ MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / EPUCK_AXLE_DIAMETER) 
    pose_theta -= (((vL / MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / EPUCK_AXLE_DIAMETER) 
    
    
    pose_x += math.cos(math.radians(pose_theta)) * ((((vL / MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2) + (((vR/ MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2)) * elapsed_time
    
    pose_y += math.sin(math.radians(pose_theta)) * ((((vL / MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2) + (((vR/ MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2)) * elapsed_time
    
    # print(math.cos(math.radians(pose_theta)) * ((((vL / MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2) + (((vR/ MAX_SPEED) * EPUCK_MAX_WHEEL_SPEED) / 2)) * elapsed_time)
